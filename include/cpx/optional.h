...
namespace cpx {
    ...
    CPX_EXPORT template <typename T, typename F>
    constexpr auto and_then(std::optional<T> &&opt, F &&f) -> decltype(std::forward<F>(f)(std::move(*opt))) {
        if (opt.has_value())
    ...
    CPX_EXPORT template <typename T, typename F>
    constexpr auto and_then(const std::optional<T> &opt, F &&f) -> decltype(std::forward<F>(f)(*opt)) {
        if (opt.has_value())
    ...
    CPX_EXPORT template <typename T, typename F>
    constexpr auto transform(std::optional<T> &&opt, F &&f) -> std::optional<std::invoke_result_t<F, T>> {
        if (opt.has_value())
            return std::forward<F>(f)(std::move(*opt));
        return std::nullopt;
    }
    ...
    CPX_EXPORT template <typename T, typename F>
    constexpr auto transform(const std::optional<T> &opt, F &&f) -> std::optional<std::invoke_result_t<F, const T &>> {
        if (opt.has_value())
            return std::forward<F>(f)(*opt);
        return std::nullopt;
    }
    ...
    constexpr std::optional<T> or_else(std::optional<T> &&opt, F &&f) {
        static_assert(std::is_invocable_r_v<std::optional<T>, F>, "or_else: F must return optional<T>");
        if (!opt.has_value())
