...
TEST(Result, ValueOrDefault) {
    ...
    auto err = Result<int>::err(std::runtime_error("fail"));
    EXPECT_EQ(ok.value_or(0), 42);
    EXPECT_EQ(err.value_or(0), 0);
}

TEST(Result, ValueThrowsOnErr) {
    auto r = Result<int>::err(std::runtime_error("bad"));
...
TEST(Result, AndThen_Ok) {
    auto r = Result<int>::ok(21).and_then([](int n) -> Result<int> { return n * 2; });
    ASSERT_TRUE(r.is_ok());
    EXPECT_EQ((int)r, 42);
}

TEST(Result, AndThen_Err) {
    auto r = Result<int>::err(std::runtime_error("fail")).and_then([](int n) -> Result<int> { return n * 2; });
    EXPECT_TRUE(r.is_err());
    EXPECT_EQ(r.error().what(), std::string("fail"));
}
...
TEST(Result, Transform_TypeChange) {
    ...
    ASSERT_TRUE(r.is_ok());
    EXPECT_EQ(std::string(r), "42");
}

TEST(Result, MapErr) {
    auto r = Result<int>::err(std::runtime_error("bad")).map_err([](const std::runtime_error &e) { return strlen(e.what()); });
    ASSERT_TRUE(r.is_err());
    EXPECT_EQ(r.error(), 3u);
}

TEST(Result, OrElse_Err) {
    auto r = Result<int>::err(std::runtime_error("fail")).or_else([](auto &) -> Result<int> { return 99; });
    ASSERT_TRUE(r.is_ok());
    EXPECT_EQ((int)r, 99);
}

TEST(Result, OrElse_Ok) {
    auto r = Result<int>::ok(7).or_else([](const std::runtime_error &) -> Result<int> { return 99; });
    ASSERT_TRUE(r.is_ok());
    EXPECT_EQ((int)r, 7);

    Result<std::string> s = "2";
}
...
TEST(Overload, VisitInt) {
    ...
    int                                   got = 0;
    cpx::visit(v, [&](int i) { got = i; }, [&](float) { got = -1; }, [&](std::string &) { got = -2; });
    EXPECT_EQ(got, 42);
}
...
TEST(Overload, VisitString) {
    ...
    std::string                           got;
    cpx::visit(v, [&](int) { got = "int"; }, [&](float) { got = "float"; }, [&](std::string s) { got = std::move(s); });
    EXPECT_EQ(got, "hello");
}
